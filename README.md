...
- buy agent 鼓勵更積極。
- 是追求獲利的版本。 
- 用fixed_lstm 每日回測和盤中觀測AI可能交易模式，並臨摹之。

# v03-04 更新

*** 現在的train_v4_models.py 不會自動把best model 複製成 final model。看是要改腳本，還是手動複製。目前用的model 是最後的版本，不是最佳的版本，但差異不大。
...
- 新增 backtest_v4_dca_hybrid_with_filter_rolling_lstm.py，方便我比較有無濾網的dca + hybrid 的差異 (start 1and 2都有濾網，但我以start 2為主要策略)。
